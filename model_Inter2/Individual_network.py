    ...
    def simulate(self):
        ...
        while(simulate_time<self.simulate_time):
            ...
            if(t_now<self.work_time[0]and t_now>=self.rest_time[1]):
                #pass
                temp_Model= network_model .Commute_Model(self.args_model,self.args_t,self.args_steps,self.Envir,self.commute_LocationList,self.home_loc,self.work_loc)
            ...
            if(t_now>self.work_time[0] and t_now<self.work_time[1]):
                #pass
                temp_Model = network_model .HomeOrWork_Model(self.args_model,self.args_t,self.args_steps,self.Envir,self.work_locatonList,self.home_loc,self.work_loc)
            ...
            if(t_now>self.work_time[1] and t_now<self.rest_time[0]):
                ...
                if(t_now<self.rest_time[0]):
                    temp_Model =  network_model .HomeOrWork_Model(self.args_model,self.args_t,self.args_steps,self.Envir,self.home_locationList,self.home_loc,self.work_loc)
            ...
            if(t_now>self.rest_time[0] or t_now<self.rest_time[1]):
                ...
                #print (simulate_time)
                #sleep
class Nomal_Network_Individual(Individual):
    # create the model location disput nomal
    # peole select home and work randomly base on the position density
    def set_home_loc(self):
        if (len(self.Envir.locations)):
    ...
    def simulate(self):
        ...
        while (simulate_time < self.simulate_time):
            ...
            if (t_now < self.work_time[0] and t_now >= self.rest_time[1]):
                # pass
                temp_Model =  network_model .Commute_Model(self.args_model, self.args_t, self.args_steps, self.Envir,
                                                  self.commute_LocationList, self.home_loc, self.work_loc)
                temp_Model.set_tbegin(t_now, self.work_time[0])
                self.commute_LocationList, tempRoute = temp_Model.get_route(0)
                self.data_mid.add_location(tempRoute.route)
                t_now = temp_Model.t_now
            ...
            if (t_now > self.work_time[0] and t_now < self.work_time[1]):
                # pass
                temp_Model =  network_model .HomeOrWork_Model(self.args_model, self.args_t, self.args_steps, self.Envir,
                                                     self.work_locatonList, self.home_loc, self.work_loc)
                temp_Model.set_tbegin(t_now, self.work_time[1])
                self.work_locatonList, tempRoute = temp_Model.get_route(1)
                self.data_mid.add_location(tempRoute.route)
                t_now = temp_Model.t_now
            ...
            if (t_now > self.work_time[1] and t_now < self.rest_time[0]):
                ...
                if (t_now < self.rest_time[0]):
                    temp_Model =  network_model .HomeOrWork_Model(self.args_model, self.args_t, self.args_steps, self.Envir,
                                                         self.home_locationList, self.home_loc, self.work_loc)
                    temp_Model.set_tbegin(t_now, self.rest_time[0])
                    self.home_locationList, tempRoute = temp_Model.get_route(0)
                    t_now = temp_Model.t_now
                    self.data_mid.add_location(tempRoute.route)
