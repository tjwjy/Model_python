...
class draw():
    ...
    rog_disput=[]
    def __init__(self,route,grid,location):
        self.routelist = route
        self.grid=grid
        self,location=location
        cal = Cal_para.Cal_para(self.routelist)
    ...
    def draw_location_disput(self):
        ...
        for i in range(len(locationWeight)):
            ...
            for j in range(len(locationWeight[0])):
                y .append(self.grid[j][0])
                x .append( self.grid[j][1])
    ...
    def draw_location_disput_raster(self):
        ...
        for i in range(len(locationWeight)):
            ...
            for j in range(len(locationWeight[i])):
                y.append(self.locations[j][0])
                x.append(self.locations[j][1])
                c.append('b')
            size+=locationWeight[i]
