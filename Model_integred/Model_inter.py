...
class ExReModel(Model):
    def dis(self,temp1,position_now):
        return math.pow((temp1.location[0]-position_now.location[0]),2)+math.pow((temp1.location[1]-position_now.location[1]),2)

    def get_next_position(self, L_place,position):
        # 概率p=size/pow(d.beta)
        beta = self.args_step[0]
        max_step = self.args_step[1]
        psum = []
