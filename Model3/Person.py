...
class normal_person(Person):
    ...
    def set_grid(self,args_grid):
        if (len(args_grid) == 3):
            temp = coorditinates.normal_raster(args_grid[0], args_grid[1],args_grid[2])
            self.grid = temp.grid
            self.locations = temp.locationList
        else:
            self.grid=args_grid[0]
            self.locations=args_grid[1]
    ...
    def simulate(self):
        ...
        while(simulate_time<self.simulate_time):
            ...
            if(t_now<self.work_time[0]and t_now>=self.rest_time[1]):
                ...
                time=[ity+simulate_time*24  for ity in tempRoute.time]
